...
from app.sydekyks.shield.playbook import PLAYBOOK_KEY
...
def seed(db: Session) -> None:
    sydekyk = db.query(Sydekyk).filter(Sydekyk.slug == "shield").first()
    if sydekyk is None:
        print("Shield seed: base Shield Sydekyk row not found yet, skipping")
        return
