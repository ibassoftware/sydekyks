...
import { api, type ShieldPlaybook } from "../../lib/api";
import { Badge } from "../../components/ui";

export function ShieldPlaybookPanel() {
  const [playbook, setPlaybook] = useState<ShieldPlaybook | null>(null);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    api.get<ShieldPlaybook>("/tenant/shield/playbook").then((res) => setPlaybook(res.data));
  }, []);
