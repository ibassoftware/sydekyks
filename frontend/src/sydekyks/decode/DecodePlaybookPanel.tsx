...
import { api, type DecodePlaybook } from "../../lib/api";
import { Badge } from "../../components/ui";

export function DecodePlaybookPanel() {
  const [playbook, setPlaybook] = useState<DecodePlaybook | null>(null);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    api.get<DecodePlaybook>("/tenant/decode/playbook").then((res) => setPlaybook(res.data));
  }, []);
