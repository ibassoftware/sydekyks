...
import { api, type NudgePlaybook } from "../../lib/api";
import { Badge } from "../../components/ui";

export function NudgePlaybookPanel() {
  const [playbook, setPlaybook] = useState<NudgePlaybook | null>(null);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    api.get<NudgePlaybook>("/tenant/nudge/playbook").then((res) => setPlaybook(res.data));
  }, []);
