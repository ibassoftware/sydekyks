...
import { api, type ScoutPlaybook } from "../../lib/api";
import { Badge } from "../../components/ui";

export function ScoutPlaybookPanel() {
  const [playbook, setPlaybook] = useState<ScoutPlaybook | null>(null);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    api.get<ScoutPlaybook>("/tenant/scout/playbook").then((res) => setPlaybook(res.data));
  }, []);
